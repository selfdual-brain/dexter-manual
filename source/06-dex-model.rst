...
**AMM price**
  ...
  ``market.ammBalanceOf(AAA) / market.ammBalanceOf(BBB)``.
...
**Swap price**
  ...
  executed conversion of tokens, we know the exact amounts of tokens sold and bought in this swap. The way swap price
  is calculated of course depends on the direction of the order, which the swap belongs to. For example for an order
  with direction ``AAA -> BBB``, the price in direction ``AAA -> BBB`` is calculated as
