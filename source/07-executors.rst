...
An executor can be seen as the manager of position's lifetime. More specifically, the goal of an executors is to
...
Within the universe of possible executor algorithms, we will say that an executor is **fair-play** if the following
conditions hold:
...
A trader ready to pay more should be served first.

**Rule #3: Longer waiting implies higher priority**

In case of a tie in rule #3, a trader waiting longer should be server first.
